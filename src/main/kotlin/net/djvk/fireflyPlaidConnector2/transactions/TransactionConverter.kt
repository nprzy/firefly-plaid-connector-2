...
class TransactionConverter(
    ...
    fun getTxDescription(tx: PlaidTransaction): String {
        val descriptionExpression = null
        ...
        return try {
            ...
            // Provide a boolean, #redundantDescription, indicating whether #merchantNameWithFallback == originalDescription
            context.setVariable("redundantDescription", merchantName.equals(tx.originalDescription, true))
            val value = parser.parseExpression(txStyle.descriptionExpression.trim()).getValue(context, String::class.java)

            value ?: run {
                logger.error("Custom description SpEL expression {} returned null. Falling-back to default.", descriptionExpression)
                defaultDesc
            }
        } catch (e: RuntimeException) {
            logger.error("Failed to parse custom description SpEL expression {}. Falling-back to default.", descriptionExpression, e)
            defaultDesc
        }
    }
